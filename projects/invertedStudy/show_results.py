...
for j, folder in enumerate(sorted(folders)):
    ...
    # ax[0].plot(X - expected_x[0], density[0, :], color="black", label="Initial")
    # ax[0].plot(X - expected_x[max_index], density[max_index, :], color="red", label="Final")
    # ax[0].axvline(0, color="red")
    # ax[0].legend()
    # ax[1].plot(T, sigma, color="black")
...
plt.figure()
plt.xlabel(r"$\xi [a_0]$")
plt.ylabel("$<x> [a_0]_{t=%.0f\\tau}$" % tau_max)
